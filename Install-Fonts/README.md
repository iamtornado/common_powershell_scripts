...
这是一个专为IT运维和系统管理设计的PowerShell脚本，能够批量安装字体文件（.ttf、.ttc、.otf）到Windows系统字体目录，自动跳过已安装的字体，并完善记录安装过程。
...
4. **重复检测**：检查字体是否已存在于系统字体目录
5. **文件复制**：将字体文件复制到 `C:\Windows\Fonts` 目录
6. **注册表注册**：在注册表中创建相应的字体注册项
7. **结果统计**：显示安装、跳过和失败的统计信息
...
4. **重复安装**：已存在的字体会自动跳过，不会覆盖
5. **系统字体目录**：字体会被安装到 `C:\Windows\Fonts` 目录
